...
export function CheckoutPage() {
  ...
  const [draft, setDraft] = useState<CheckoutDraft>({ name: '', phone: '', email: '', deliveryType: 'DELIVERY', address: '', neighborhood: '', reference: '', paymentMethod: 'CASH', cashTendered: '', pointsToRedeem: '' })
  const [couponInput, setCouponInput] = useState(''); const [appliedCoupon, setAppliedCoupon] = useState('')
  ...
  useEffect(() => {
    ...
    const controller = new AbortController(); const timer = window.setTimeout(() => {
      ...
      quoteOrder({ deliveryType: draft.deliveryType, lines: buildCheckoutLines(cart.lines, false), customerPhone: draft.phone.trim().length >= 7 ? draft.phone.trim() : null,
        couponCode: appliedCoupon || null, pointsToRedeem: draft.pointsToRedeem ? Number(draft.pointsToRedeem) : 0 }, controller.signal)
  ...
  async function submit(event: FormEvent) {
    ...
    const request: CreateOrderRequest = {
      ...
      couponCode: appliedCoupon || null, pointsToRedeem: draft.pointsToRedeem ? Number(draft.pointsToRedeem) : 0,
    }
    setSubmitting(true)
    try {
...
export function CheckoutSummary({ quote, error, quoteError, retryQuote, submitting, currency }: { quote: CheckoutQuote | null; error: string; quoteError: string; retryQuote: () => void; submitting: boolean; currency: string }) {
  const cart = useCart()
  return <aside className="checkout-summary"><p className="eyebrow">Tu pedido</p><h2>{cart.units} {cart.units === 1 ? 'producto' : 'productos'}</h2><div className="checkout-summary__lines">{cart.lines.map((line) => <article key={line.id}><ProductImage src={line.imagePath} alt="" /><div><strong>{line.quantity} × {line.name}</strong>{line.extras.length > 0 && <small>+ {line.extras.map((extra) => extra.name).join(', ')}</small>}{line.notes && <small>“{line.notes}”</small>}</div></article>)}</div><div className="totals">{quote ? <><div><span>Subtotal</span><strong>{formatMoney(toMinorUnits(quote.totals.subtotal), currency)}</strong></div>{quote.loyalty.couponDiscount > 0 && <div className="discount"><span>Cupón {quote.loyalty.couponCode}</span><strong>− {formatMoney(toMinorUnits(quote.loyalty.couponDiscount), currency)}</strong></div>}{quote.loyalty.pointsDiscount > 0 && <div className="discount"><span>{quote.loyalty.pointsRedeemed} puntos</span><strong>− {formatMoney(toMinorUnits(quote.loyalty.pointsDiscount), currency)}</strong></div>}{quote.totals.discount > quote.loyalty.couponDiscount + quote.loyalty.pointsDiscount && <div className="discount"><span>Promoción</span><strong>− {formatMoney(toMinorUnits(quote.totals.discount - quote.loyalty.couponDiscount - quote.loyalty.pointsDiscount), currency)}</strong></div>}<div><span>Domicilio</span><strong>{quote.totals.deliveryFee === 0 ? 'Gratis' : formatMoney(toMinorUnits(quote.totals.deliveryFee), currency)}</strong></div><hr /><div className="grand-total"><span>Total</span><strong>{formatMoney(toMinorUnits(quote.totals.total), currency)}</strong></div><small>Ganarás {quote.loyalty.pointsToEarn} puntos cuando el pedido sea entregado.</small></> : quoteError ? <p>No pudimos actualizar los precios.</p> : <p>Actualizando precios…</p>}</div>{(error || quoteError) && <div className="checkout-error" role="alert">! {error || quoteError}</div>}{quoteError && <button className="button button--light button--wide" type="button" onClick={retryQuote}>Reintentar precios</button>}<button className="button button--acid button--wide checkout-submit" disabled={submitting || !quote}>{submitting ? 'Creando pedido…' : 'Confirmar pedido →'}</button><p className="secure-note">El pedido solo se crea una vez, incluso si reintentas.</p></aside>
}

...
export function AdminDashboardPage() {
  ...
  if (!data) return <AdminLoading label="Preparando el panel…" />
  const cards = [['Pedidos hoy', data.ordersToday, 'Actividad del día'], ['Ventas hoy', money(data.salesToday), 'Pedidos no cancelados'], ['Nuevos', data.newOrders, 'Requieren atención'], ['En preparación', data.preparingOrders, 'En cocina'], ['Pagos por revisar', data.paymentsUnderReview, 'Transferencias'], ['Stock bajo', data.lowStockIngredients, 'Revisar inventario'], ['Agotados', data.outOfStockIngredients, 'Sin existencias'], ['Ticket promedio', money(data.averageTicket), 'Promedio del día']]
  return <><PageTitle eyebrow="RESUMEN OPERATIVO" title="Panel general" action={<button className="secondary" onClick={load}>Actualizar</button>} />
